...
def stochastic_conditioning(x):
    """
        Picks a random view at each diffusion step. 
        Inspired from: https://arxiv.org/abs/2210.04628
    """
    if type(x) == list:
        index = torch.randint(0,len(x), (1,))[0]
        return x[index]
    # Only when we have lists of embeddings (multi views) => stochastic conditioning
    else:
        return x
...
class CLIPImageGridPointDiffusionTransformer(PointDiffusionTransformer):
    ...
    def cached_model_kwargs(self, batch_size: int, model_kwargs: Dict[str, Any]) -> Dict[str, Any]:
        _ = batch_size
        with torch.no_grad():
            # Multi view => list of embeddings
            if len(model_kwargs["images"]) > 1:
                embds = [self.clip.embed_images_grid([i]) for i in model_kwargs["images"]]
            else:
                embds = self.clip.embed_images_grid(model_kwargs["images"])

            return dict(embeddings = embds)
    ...
    def forward(
        ...
        if images is not None:
            ...
            embeddings = stochastic_conditioning(embeddings)
            clip_out = embeddings
...
class CLIPImageGridUpsamplePointDiffusionTransformer(UpsamplePointDiffusionTransformer):
    ...
    def forward(
        ...
        *,
        ...
        if images is not None:
            images = stochastic_conditioning(images)
            clip_out = self.clip.embed_images_grid(images)
