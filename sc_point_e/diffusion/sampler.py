...
class PointCloudSampler:
    ...
    def sample_batch_progressive(
        ...
        for (
            ...
            # Compute cached model kwargs (almost always) => embeddings
            ...
            if stage_guidance_scale != 1 and stage_guidance_scale != 0:
                for k, v in stage_model_kwargs.copy().items():
                    # Adapted to multi view list of embeddings
                    if k == "embeddings" and len(model_kwargs["images"]) > 1:
                            stage_model_kwargs[k] = [torch.cat([i, torch.zeros_like(i)], dim=0) for i in v]    
                    else:
